...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/25 19:12:57 by blinnea           #+#    #+#             */
/*   Updated: 2019/10/25 19:13:32 by blinnea          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_err	puttetr(t_tetr *tetr, t_field *map, t_coord place)
{
	...
	if (tetr->dright.x - tetr->uleft.x + place.x >= map->size || \
			tetr->dright.y - tetr->uleft.y + place.y >= map->size)
		return (ERR);
	y = -1;
	while (++y <= tetr->dright.y - tetr->uleft.y)
	{
